...
| 测试 | Node 原生 test runner（75 用例） + 五个高仿真表单页回归 |
...
```bash
npm test                    # 75 个纯逻辑单测
node tools/serve.js         # 本地 8123：五个高仿真表单页回归
```
...
以下功能代码完整保留，设置页暂不展示（`sidepanel.html` 注释处删除 `<div hidden>` 即可启用）：

- **强力填充**：`chrome.debugger` 派发真实鼠标/滚轮/键盘事件，自动选中合成事件搞不定的下拉。填充期间页面顶部会短暂出现调试横幅，填完自动断开。
- **视觉兜底**：低置信度字段截图后交视觉模型重映射再补填，视觉模型留空则复用文本模型配置。
